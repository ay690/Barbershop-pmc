  ...
  return (
    <section id="services" className="py-20 bg-gray-50 dark:bg-gray-900 transition-colors duration-500">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`grid grid-cols-1 ${isMobile ? "" : "sm:grid-cols-2 lg:grid-cols-3"} gap-8`}>
          {services.map((service, index) => (
            <div
              key={index}
              className={`bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 transform ${
                fadeIn ? "fade-in opacity-100 translate-y-0" : "opacity-0 translate-y-5"
              }`}
              style={{ transitionDelay: `${index * 0.1}s` }}
            >
              <div className={`text-gold ${isMobile ? "text-3xl" : "text-4xl"} mb-4`}>
                {service.icon}
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <a
                  href="#book"
                  className="text-gray-800 dark:text-gray-200 hover:text-gold dark:hover:text-gold font-medium"
                >
                  Book Now
                  <svg
